...
> 带编译期不重叠断言），并把 `common/logger.h` 从**全 no-op 存根**升级为 **stderr 最简实现**
...
**`common/logger.h`（当前）**：全 no-op 宏，五级（`DEBUG/INFO/WARN/ERROR/FATAL`），但头部注释只列了 4 级语义（漏 `INFO`）。
```cpp
...
- logger 是全 no-op —— ROADMAP P0 明确"**禁止全 no-op**"，M3 必须实装 stderr。
...
| 本设计裁决 | **纯头宏 + `printf` 风格 `std::fprintf`**：沿用原 `logger.h`「纯头、宏」的最简形态，仅把 no-op 宏体换成单次 `fprintf` 到 stderr。无 `.cpp`、无模板、无 `std::format`。`fmt` 须为字符串字面量（与前缀拼接）；用 C++20 标准 `__VA_OPT__` 转发变参（不用 GNU `##__VA_ARGS__`，`-Wpedantic` 干净） |
...
- `FATAL` 仅**输出**，不主动 `abort()`：logger 单一职责是"记日志"，是否终止进程由调用方 / `assert` /
  上层策略决定（避免日志库越权控制流程）。需要 fail-fast 的点用 `assert` 或显式 `std::abort()`。
