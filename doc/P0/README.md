...
| 里程碑 | 主题 | 设计稿 | 状态 |
|---|---|---|---|
| M1 | 项目骨架与构建系统 | [P0M1_skeleton_design.md](P0M1_skeleton_design.md) | ✅ 已锁定（P0M7 收敛） |
| M2 | `common/structs.h` schema 定型 | [P0M2_schema_design.md](P0M2_schema_design.md) | ✅ 已锁定（P0M7 收敛） |
| M3 | 错误码段位规划 + Logger stderr 实装 | [P0M3_errorcode_logger_design.md](P0M3_errorcode_logger_design.md) | ✅ 已锁定（P0M7 收敛） |
| M4 | `util/hash.{h,cpp}` xxh3 接入 | [P0M4_hash_design.md](P0M4_hash_design.md) | ✅ 已锁定（P0M7 收敛） |
| M5 | 测试与 bench 框架接入 + util/common 覆盖 | [P0M5_test_bench_design.md](P0M5_test_bench_design.md) | ✅ 已锁定（P0M7 收敛） |
| M6 | 本地内存检测器组合矩阵 + 测试 / 覆盖率脚本（CI 推迟） | [P0M6_test_scripts_design.md](P0M6_test_scripts_design.md) | ✅ 已锁定（P0M7 收敛） |
...
M2 / M3 / M4 互不依赖，可并行；建议按 M2 → M3 → M4 串行提 PR 便于审阅。
...
- 阶段收敛稿：`P<阶段>M<最后里程碑>_convergence_design.md`（如 `P0M7_convergence_design.md`）
