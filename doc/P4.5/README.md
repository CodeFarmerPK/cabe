...
> 默认实现为固定大小环形队列（FIFO）。为 P5 崩溃恢复提供重建接口；P7 通过 reactor
> 单线程所有权和多设备分区复用该分配器，不要求分配器内部原子化。
...
3. **三容器的核心优势在无锁分离**：但 P4.5 阶段仍为单线程。P7 最终采用 per-reactor
   状态独占，每个环形队列仍只由所属 reactor 单线程访问，因此无需把 `head` / `tail` 改成原子变量。
4. **复杂度不匹配**：三容器轮换 + 后台排序线程的实现和测试成本高，用于解决一个
   "对 1M 块无价值"的问题，投入产出比不合理。

### 为什么选择环形队列（FIFO）

1. **FIFO 语义优于 LIFO**：P4.5 改造前的 LIFO 方案会导致热块反复使用——刚释放的块立刻被重新分配；
   FIFO 让块使用更均匀——刚释放的块排到队尾，等其他块用过一轮才轮到，对 SSD 磨损均衡更友好。
2. **固定大小**：设备块数在 Open 时已知且不变，环形队列一次分配、零碎片。
3. **O(1) 操作**：获取和回收都是常数时间。
4. **并发边界简单**：P7 通过所有权分区让每个分配器保持单线程实现；公开并发路径无锁，
   分配器内部不承担跨线程同步。
...
- TRIM 占位：Engine::Delete 路径预留调用点；P7 只将其迁入 reactor，真实异步批量 TRIM 继续归性能兑现阶段
...
2. ✅ owner 已确认启动
3. ✅ P4.5M1～P4.5M3 详细设计、实现与收敛全部完成

## 已锁定决策

| 编号 | 决策 | 结果 |
|---|---|---|
| P4.5-D1 | TRIM 触发位置 | Engine 层（方案 B）——块分配器保持纯数据结构，不碰 I/O。P7 将空桩迁入 reactor；真实异步批量实现继续归性能兑现阶段 |
...
**设计前问题及后续裁决**：
1. P4.5 当时曾询问 `Init` 是否要为 P5 超级块改成从块 1 开始。P5M1 最终两个候选都未采用：
   P5-D8 使用设备头部独立 8K（双份 4K），逻辑块仍从 0 编号，物理偏移由 IoBackend 增加
   `kDataRegionOffset`；`Init` / `RebuildFromActive` 零调整，分配器全程不感知超级块。
...
4. **P7 并发兑现路径**：每个分配器由所属 reactor 单线程独占，`head` / `tail` 保持普通变量；
   无锁性由 MPSC 投递和状态所有权分区保证。
5. **TRIM 职责分离**：块分配器只管块号（纯数据结构），TRIM 由 Engine 层负责。
   当前仍为空桩；P7 只完成调用点迁移，异步批量发送归性能兑现阶段。
6. **块分配器不持久化**：恢复时从 MetaIndex 反推——FreeList 重建是微秒到毫秒级纯内存操作，
   在整体恢复（WAL 重放为秒级磁盘 I/O）中占比可忽略。持久化的复杂度（一致性维护、额外 I/O）
   远大于节省的恢复时间。
7. **超级块（P5）**：超级块占设备头部独立 8K 预留区（非数据块），逻辑块仍从 0 编号；
   物理偏移由 IoBackend 增加 `kDataRegionOffset`（P5-D8），`Init` / `RebuildFromActive` 均不感知超级块。
