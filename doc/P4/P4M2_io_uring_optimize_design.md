...
| 推迟项 | 落点 | 原因 |
|---|---|---|
| 预注册缓冲区（registered buffers） | **P8 已兑现** | P4 时接口只传内存地址；P8M4 最终以 `RegisterWriteBuffers`、`IoWriteBuffer` 和槽位身份完成实现 |
| 批量提交 | **性能兑现阶段** | P7 建立 reactor 后仍是每次提交即等待，没有实现多请求在飞 |
...
| 编号 | 决策 | 结果 | 理由 |
|---|---|---|---|
| **P4M2-D1** | 优化范围 | P4 只做预注册文件描述符；预注册缓冲区推到 P8 | P4 时接口不传缓冲区索引；P8M4 后已由 value 专用缓冲区池和内部写入描述符兑现 |
...
| 预注册缓冲区推迟到 P8 | P4 不覆盖这部分优化 | P4 只验证提交/等待路径和预注册文件描述符，不据此声称性能收益；P8 再统一实现注册缓冲区与零拷贝路径 |
...
| 下游 | 接入点 |
|---|---|
| **P4M3** | 在预注册文件描述符基础上做 TSAN 兼容验证和部署收敛；最终不做性能基准归档 |
| **P8** | 已通过 value 专用 `ValueBuffer` / `ValueBufferPool` 和内部 `IoWriteBuffer` 落地注册缓冲区写入；公开 `Put` 未分裂，旧占位名 `BufferHandle` 未采用 |
