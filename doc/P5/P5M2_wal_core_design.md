...
> 与管理数据设备的 `IoBackend` 分离。本里程碑只做级别 1；级别 2/3/4 在 M3 叠加，崩溃恢复/重放在 M5。
>
...
| 下游依赖本里程碑 | P5M3（WAL 级别 2/3/4，复用 `Wal` 接口）、P5M5（恢复重放，复用帧格式 + 解码） |
| 退出判定 | 见 §12 |
...
10. **关联文档**：更新 `doc/P5/README.md`（M2/M5 范围重划）、`ROADMAP.md`（P5 章节）。
...
| 环形缓冲区（绕圈覆盖 + 头部回收 + 写满兜底） | P5M4 | 回收依赖快照；M2 只做线性追加，假定 WAL 不写满 |
| 崩溃恢复 / WAL 重放 / `RebuildFromActive` | P5M5 | M2 的 WAL 是"只写"；读回重建索引在 M5 |
| recover 模式下的 WAL（重放已有日志 + 续写） | P5M5 | M2 的 WAL 追加只做在 create 模式 |
...
| 编号 | 决策 | 结论 |
|---|---|---|
| **P5M2-D1** | M2 范围（M2/M5 边界） | **端到端级别 1**：`wal` 模块 + 帧 + 把级别 1 接进 `Engine`。级别 2/3/4 → M3；重放/恢复 → M5。（重划了 roadmap 原来"Engine 接线在 M5"的写法） |
...
| **P5M2-D8** | 写满兜底 | M2 **不判 WAL 满**(线性只增,假定不满);环形缓冲 + 头部回收 + 写满兜底 → M4。自然兜底:写过设备尾 `RawDevice` 返回 `kIoBase`(失败安全,不静默损坏) |
| **P5M2-D9** | create/recover | WAL 追加只做在 **create 模式**(从偏移 8K 开空日志,`seq` 从 1);recover 的重放 + 续写 → M5 |
...
- `magic` / `version`：识别帧 + 格式版本。读回时魔数或版本不符 → 视为非本格式帧（M5 重放据此定边界）。`version` 是**帧格式版本**，与"快照代际"无关。
...
- `frame_crc32c`：覆盖 [0,124),校验帧自身完整(检测半截写入 + M5 重放定边界)。
...
M2 日志区只增不减（回收在 M4），从 8K 线性往后填。M2 **不判满、不绕圈**,假定不会写满（mkloop 的 WAL 16 MiB ≈ 12 万帧,测试用不满）。若假定失效真写过设备尾,`RawDevice.WriteAt` 返回 `kIoBase`——失败安全,不静默损坏。
...
Engine::Open(opts)：
  ...
  // recover 模式：M2 不开 WAL 追加（重放/续写 → M5）
```
...
- **崩溃时内存整块消失**;恢复(M5)从 WAL 重放重建索引——**WAL 是唯一真相,内存是可重建的缓存**。
- 任意崩溃点:已 ack 的写/删,其 WAL 帧已持久,恢复后状态一致;未 ack 的,丢失即可(调用方未收到成功)。
- 数据盘上残留的旧 value 字节**不算不一致**:一个块是否"活"由(重放出的)索引决定,没有索引指向它 = 空闲垃圾,`RebuildFromActive`(M5)从活索引反推空闲块,自然把它算空闲。
...
前提：重放/恢复在 M5,故 M2 只测"帧是否正确写出 + 写路径是否通",不测"崩溃恢复"。
...
**不测**（推迟）：崩溃恢复/重放（M5）；故障注入（断电、EINTR、熵失败）。
...
7. 关联文档更新：`doc/P5/README.md`（M2/M5 范围重划）、`ROADMAP.md`（P5 章节）。
