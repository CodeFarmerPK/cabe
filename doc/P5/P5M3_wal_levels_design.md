...
| 下游依赖本里程碑 | P5M4（快照：检查点时复用 `Flush()` 刷净 WAL）、P5M5（恢复：复用帧 + 双 CRC + `seq`） |
| 退出判定 | 见 §13 |
...
| 崩溃恢复 / WAL 重放 / value 损坏的恢复策略 | P5M5 | M3 无恢复；级别 3/4 崩溃后的损坏检测靠现有 `Get` CRC，恢复处置在 M5 |
| 环形缓冲 / 回收 | P5M4 | M3 仍线性追加，假定 WAL 不写满 |
...
| **P5M3-D5** | 缓冲结构 | **一块**缓冲，大小 = `wal_buffer_size`，Open 时定、一次性分配、运行期固定，Open 时向上取整到 4K 倍数且 ≥4K；两档共用；空闲部分不管；对不完整写入安全（帧 CRC 兜底） |
...
| **P5M3-D10** | 读侧 | M3 读路径不加新代码；级别 3/4 崩溃后的 value 损坏由现有 `Get` 的 CRC 兜住；处置策略 → M5 |
...
**崩溃一致性**（恢复是 M5，但级别设计现在就要自洽；下表写进文档）：
...
- **级别 3（默认）**：反过来——key 永不丢（WAL 同步），但最近 value 可能没落盘，崩溃后读出 CRC 不匹配。这就是"键在值坏"，检测靠现有 `Get` CRC（§9），处置留 M5。
...
- **攒批 → 同步（收紧）**：由 Engine 改级别入口在改级别前先调 `Flush()`，保证"同步即落盘"从此刻起成立，缓冲回到干净状态；
- **同步 → 攒批（放松）**：无需特殊动作——同步档的帧本就已落盘，后续攒批的整块刷会把它们一起重写（同字节、幂等）。

### 6.7 对不完整写入安全

刷出写的是整数个 4K 块、每块都是完整帧。断电刷到一半：没写到的尾块 → 那些帧本就未确认、丢失正确；最多一个被写一半（撕裂）的块 → 帧 `frame_crc` 不过 → 恢复时（M5）当"没写过"。与级别 1 的"重写整块"同一套 CRC 兜底。
...
- **掉电崩溃 + 恢复后（M5）**：WAL（级别 3 同步）重放出 key→块，但块 value 没落盘 → 块里是旧数据/垃圾 → `Get` 读出 CRC 不匹配 → `kEngineDataCorrupted`。
- **M3 读路径不加任何新代码**：现有 `Get` 的 CRC 校验就是检测路径，不区分"没落盘 / 真损坏"，都返回 `kEngineDataCorrupted`。
- M3 没有恢复、不崩溃，这个场景在 M3 **跑不出来**（要故障注入，已推迟），真正验证在 M5；损坏处置策略（丢弃 / 报上层 / `verify_value_crc_on_recovery`）也在 M5。
...
- **测不了、推迟**：崩溃下各级别真正丢什么（→ M5 故障注入）；value 的 FUA vs 异步**持久性**（崩溃才显形 → M5 / 代码审查）。
...
- **并发安全的运行时改级别** → P7；**环形缓冲 + 回收** → M4；**崩溃恢复 / 重放 / value 损坏的恢复处置** → M5。
