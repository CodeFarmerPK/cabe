...
> **P6M1 起本节已超越**：同步档改走提交组（group commit）——并发写者的帧由轮值
> leader 合并成批，复用攒批档的窗口打包 + 段写机械，**批末一次 fdatasync**；"单块
> 暂存"表示法消失，两档统一为一套窗口表示法、差异收窄为 fsync 时机 + 返回时机两个
> 策略。**单线程下逐调用语义等价**（每调用自成一批、批末必刷、返回即持久、盘面字节
> 相同）；级别语义表（§3/D4）的持久化承诺不变。详见 `P6M1_commit_group_design.md` §5。
...
（P6M1 兑现注：同步档批量提交后本句仍成立，论证升级——每批以 fdatasync 收尾 ⇒
已分配 seq 的帧皆持久；缓冲内的变体 Y 留窗帧只是盘上字节的副本、非欠账。另：P6M1
将 `Flush` 拆为 `WriteOutWindow`（写出+搬运+重开窗）+ `Sync` 的组合，对外行为恒等。）
