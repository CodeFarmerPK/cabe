...
（P5M6 兑现注："凭扫描推回"落地为两遍扫描——census（全环普查，O(1) 内存）+ 循序走读；head = AlignDown(最小活帧偏移, 4K)、tail 锚定重放集末帧所在块、`seq_next` = 锚帧+1。派生闭环：崩在"快照成功后、`ReclaimUpTo` 前"时丢失的回收被恢复自动补完——回收纯内存的设计连"执行与否"都无需被记住。见 P5M6-D10/D17。）
...
（P5M6 兑现注：三分类在恢复中真实落地——census 逐槽分类即本表；M6 并在其上加了恢复侧对偶条款"盘上活区间必须 seq 稠密"（走读单判据"下一槽=下一 seq"的成立前提），由撕裂碎片抹除 + 锚帧+1 稠密续号共同维护。见 P5M6-D12/D13/D17 与 §10.3 纠正存照。）
...
> **P6M1 注**：同步档改走提交组后，本表"同步档"行的检查点**作废**——`Append` 的块
> 推进分支随之删除，同步档与攒批档的检查点统一为 leader 批内逐帧准入（窗口 0 → 惰性
> `WindowAt`；窗口满 → 写出后重开），W 一律 = 有效窗口；"帧未编码、seq 未耗"的干净
> 拒绝性质与惰性开窗语义**原样保持**（救援重试的成立前提不变）。同步档盘面几何自此与
> 攒批档同构（留窗块 + 紧凑活区间），§7 回收边界语义对两档逐字成立。见 P6M1 稿 §5。
...
（P5M6 注：四条不变量在恢复侧逐条验收并跨重启存续——①head 重建 = covered 的物理孪生重推；②重建只取崩溃前活区间子集；③走读单判据盘上验收 + 留窗重灌（禁跳块）；④走读验收 + 稠密续号。并新增 ④ 的恢复侧对偶条款：**盘上活区间 seq 稠密**。验收表见 P5M6 §8.3。）
...
（P5M6 兑现注：M6 已落定上述全部推迟项——接口形态为 `Open` 公共化 + 新增 `Recover(covered_seq, WalReplayFn)` 两段式；`kEngineNotImplemented` 守卫随恢复编排落定而退场。"M5 结束时行为与 M4 相同"是当时的时点判定，自 M6 起被设计性取代。见 P5M6 稿。）
