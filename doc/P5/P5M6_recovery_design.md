...
| 下游依赖本里程碑 | P5M7（收敛，纯文档）；P7 复用恢复链并接入多设备；P9M12 在全 SPDK 模式复用同一恢复算法 |
| 退出判定 | 见 §16 |
...
| 模糊快照 / 后台恢复优化 / per-device 并行恢复 | 性能兑现阶段 / P11 | P7 支持多设备但 Open 恢复仍按现有编排执行；模糊快照未实现 |
| TRIM（含恢复路径感知 discard 区间） | 性能兑现阶段 | P7 未实现；未来实施时必须重新对账 §6.2 census 的“帧从不被清除”前提 |
...
| **P5M6-D11** | 普查 | M5 §6.3 三分类落地（`VerifyFrame` 不过 → 无效；过且 seq > covered → 活帧；否则残留）。采集五标量：`live_count`、`min_live_seq`+偏移、`max_live_seq`+偏移、`max_valid_seq`+偏移（**全局**，无活帧时 = 续写锚）、`valid_count`。census 层裁决一条：全环无合法帧 + covered>0 → 拒开（帧从不被清除；未来性能阶段实施 TRIM 时此前提必须重新对账）。重复 seq 不单查——由 D13 碎片规则间接拒掉 |
...
> **P6M1 注**：D13 的"碎片唯一合法来源 = 最后一次撕裂的**攒批刷出**"自 P6M1 起扩展为
> "最后一次撕裂的**窗口段写**"——同步档（级别 1/3）改走提交组批量提交后，其写出复用
> 攒批档的窗口机械，崩溃撕裂形态与攒批刷出**同构**（范围 ≤ 一窗且不跨缝、fdatasync
> 屏障 ⇒ 至多一次、必在尾）。本节双条件容差与抹除流程**零改动**自动覆盖；"规则级别
> 无关"的预判由此兑现。盘面等价论证见 P6M1 稿 §5.3。
...
实装与本稿 D1~D20 逐条吻合（§13 矩阵 + 既有全量回归全绿实证），偏差仅四笔注记级：

1. **bench 重开语义改 create**（交付范围 §1.2 未预列）：M6 后 recover 是真恢复，写满后 recover
   重开会回到"索引满 + 空闲零"、Put 永远 `kEngineNoSpace`——旧"重置 FreeList"语义随真恢复消亡，
   `engine_bench.cpp` 的 BM_Put 重开改 create 重新格式化（含注释）。
2. **direct-Wal 测试夹具语义对齐**：`Wal::Open` 分模式后，"开空环写入"须显式 `create=true`——
   `MakeWalOpts` 补之，并新增 `MakeWalRecoverOpts`（§13 测法的实装细化）。
3. **测试矩阵 27 例 → 实装 30 例**：`LoadRejectsBadRecord` 按校验分层拆为快照级内存安全守卫用例
   （key_len > 96）与引擎级统一校验表用例（key_len = 0）；slots 负半边 2 例独立计数。
4. 一处测试助手笔误（void 助手误接流式断言），编译期即暴露、当场修正，无设计含义。

完整对账见 [P5M7_convergence_design.md](P5M7_convergence_design.md) §7；收口验证实证见其 §8。
