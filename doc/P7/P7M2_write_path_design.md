...
5. **`wal/wal.h`+`wal/wal.cpp`**、**`snapshot/snapshot.h`+`.cpp`**、**`io/sync/sync_io_backend.h`+`.cpp`**、
   **`io/uring/io_uring_backend.h`+`.cpp`**:各加一行 `void RebindOptions(const Options* p) noexcept { opts_ = p; }`。
...
| `dc.wal`(Wal) | `wal.h:177` 的 `opts_` | `wal_level` → `IsWalSyncLevel` 决定同步/攒批 |
| `dc.snapshot`(Snapshot) | `snapshot.h:79` 的 `opts_` | 只读 `snapshot_buffer_size`(**不读 wal_level**) |

三者目前在 Open 第一阶段被设成 `&Engine::options_`(`engine.cpp:55/73/75/181/193`)。M2 要把它们重指到
本 reactor 的配置拷贝。
...
void RebindOptions(const Options* p) noexcept { opts_ = p; }
```
...
Finalize(op, rc);   // result.store(release) + 唤醒 g_wake_gen(同 M1,扣 last-touch)
```
...
(reactor `result.store` 后只碰 `g_wake_gen`、不再碰 OpNode,§11.1)。
...
1. **栈节点逐轮复用 / 写 op 安全靠 last-touch 条款**:广播每轮新建栈 `OpNode`、reactor `Finalize` 在
   `result.store(release)` 之后只碰 `g_wake_gen`(thread_local 长寿字)、**不再碰 OpNode**,故调用方一读到
