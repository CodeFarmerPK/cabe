...
2. **三竞态实证**:运营 op(SetWalLevel / Snapshot / Close)与并发 Put 在 reactor 串行下"竞态消失"的实证。
...
| 机制 | 位置 | 多调用线程下为何已安全 |
|---|---|---|
| 入队 push | `reactor.cpp:50` `Submit`(`compare_exchange_weak`,`reactor.cpp:54`) | Treiber 多生产者无锁 push:冲突即重载 `old` 重试;成功的 release 发布 op 内容(§5) |
| 整批摘 drain | `reactor.cpp:59` `DrainAll`(`exchange(nullptr, acquire)`,`:60`) | 单消费者;acquire 看到所有 push 的 release(release-sequence,§5) |
| 结果发布 + 唤醒 | `reactor.cpp:74` `Finalize` | 按每 op 的 `wake` 指针精确唤醒对应 caller;`result.store(release)` 为对 op 的最后一次访问(last-touch) |
| 空闲等待 | `reactor.cpp:99` `Run` / `:103` `inbox_.wait(nullptr, acquire)` | 原子 wait 自带"检查空与阻塞"原子;并发 push 的 notify 唤醒,无丢唤醒 |
| 调用线程等待 | `reactor.cpp:276` `SubmitAndWait` | 每 caller 用自己的 `thread_local g_wake_gen`、自己的栈 `OpNode`;投递后只碰本线程 wake 字 + 自己的 `op.result` |
...
| 唤醒 → caller wait | `wake fetch_add` **release** + notify / `wait` **acquire** + 双检 result | 够(wake_gen 是 caller 自己的 thread_local,单写单读) |
...
| **ConcurrentWritesRecoverConsistent** | N 线程并发 Put/Delete 不相交 key 到**已知终态**,join → Close → 重开 recover | ③ | 每个应存 key 读回应有值、已删 key 缺席 |
...
- **多轮**:用例内 2~3 轮 + 手动多跑二进制(README"sync+TSAN 多轮");内轮别太多以免 TSAN 测时爆。
...
2. §三竞态在 reactor 串行下消失的实证(SetWalLevel/Snapshot/Close 与并发 Put 交错,结果一致不崩)
