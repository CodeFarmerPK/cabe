...
3. **`test/engine/reactor_test.cpp`** 与 **`test/engine/engine_test.cpp`**(均改):各一处 `MultipleDevicesFails` → `TooManyDevicesFails`(§6)。
4. **`test/engine/engine_multidevice_test.cpp`**(新建,主工作量):4 个多设备用例(§7;文件后由 P7M5 隔离收敛追加第 5 个用例 `OpenFailsCleanlyOnBadDevice`,归属 P7M5,见 P7M5 设计 §交付物)。
...
| 不做项 | 归处 |
|---|---|
| 运营口并行广播 scatter-gather(`SubmitAndWaitAll`) | 性能轮(每 caller 一个进程级 WakeSlot 槽、跨 reactor 共享的基础已就位,留干净增量) |
...
> **拓扑防线互补**:这里的 `device_id` 校验防"组内下标漂移"(某盘符换了位置);P7 收敛期又补了 `device_count`
> (设备组总数 N)校验作为互补防线——用更短/更长的设备列表 recover 会被拒(错误码 `kSuperBlockDeviceCountMismatch`,
> 与 device_id 同持久于 super_block、见 P5M1)。二者一防组内下标、一防组总数,合起来封住设备列表的漂移与缩/增。
...
- 并行 `SubmitAndWaitAll`(先 scatter 全部、再在同一 WakeSlot 上 gather 等齐、取首错):~30 行新代码 + 新 TSAN
  测试面;且 Close 是 Stop+join 不是 SubmitAndWait,**套不进** SubmitAndWaitAll(硬上会让三口处理不一致)。

合 P7-D2"首轮不纠结性能"。每 caller 一个进程级 WakeSlot 槽、跨 reactor 共享的基础**已就位**,将来加 `SubmitAndWaitAll` 是干净局部增量、
...
`reactor_test.cpp` 与 `engine_test.cpp` 各有一处 `MultipleDevicesFails`(分别为 `TEST(Reactor, …)` 与 `TEST(Engine, …)`,验"N=2 必被拒")编码旧不变量——放开后 2 设备合法(会去开假路径
失败、返 IO 错而非 `kEngineInvalidOpts`),两处都必须改。均改成 **`TooManyDevicesFails`**:构造 257 个假设备 `Open`
→ `kEngineInvalidOpts`(reactor_test 用 257 个假 `DeviceConfig`、engine_test 用 257 次 `push_back`;size 校验在开设备之前、不碰假路径),守 N≤256 上界。
...
> 注:本表为 M4 交付的 4 个用例;同文件第 5 个 `OpenFailsCleanlyOnBadDevice` 由 P7M5 追加(隔离演示),归属 P7M5。
...
| **MultiThreadMultiDevice** | ⑤ + 复合 | N 调用线程并发 Put/Get/Delete 不相交 key(跨两设备)→ sync+TSAN race-free → 先 join 再 Close;QPS 仅作概念性观察项、本轮不在用例中落任何日志(loop 盘数不可信,真度量留 P11),用例只验正确性+不挂 |
...
TSAN 下**实测确认推论**,不重测 M3/M4。它对应 exit ⑤,但 QPS 仅作概念性观察、本用例不落任何 QPS 日志(真度量留 P11)。一个即够、不做重。
...
5. (观察)QPS 随设备数扩展——仅作概念性观察项,本轮用例不落任何 QPS 日志(loop 盘数不可信),真盘度量留 P11
