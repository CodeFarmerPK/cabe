...
✅ **已实施**（阶段设计 + 五里程碑详细设计与代码全部完成；P7M5 收敛通过——见 [P7M5_isolation_convergence_design.md](P7M5_isolation_convergence_design.md)）
...
- **同步包异步**：调用线程算路由 → 栈上建 op → 投递入站 MPSC 队列 → 挂调用线程的长寿 WakeSlot 槽（进程级，非 thread_local） →
...
| 里程碑 | 主题 | 设计稿 | 工作量 | 状态 |
|---|---|---|---|---|
| M1 | reactor 机制（读路径）+ 同步包异步 | `P7M1_reactor_skeleton_design.md` | L | ✅ 已实装 |
| M2 | 写路径入 reactor | `P7M2_write_path_design.md` | M | ✅ 已实装 |
| M3 | 单设备多线程 | `P7M3_single_device_mt_design.md` | M | ✅ 已实装 |
...
3. ✅ 五里程碑详细设计 + 代码全部完成（M1~M5，P7M5 收敛）
...
| P7-D6 | 同步包异步唤醒 | futex（C++20 `atomic::wait/notify`，复用 P6）。**唤醒挂点 = 每调用线程一个进程级长寿 WakeSlot 槽（`CallerWakeWord` 首次分配、CAS 注册进静态侵入链表 `g_wake_registry`、永不释放但经 registry 始终可达）、与 per-op 结果槽分离**；OpNode 只带指向该槽 `gen` 的指针。**为何是进程级永生槽、不是 thread_local**：thread_local 寿命 = 线程，快路径下调用线程一读到 result 即返回并退出，reactor 随后对已析构 thread_local 做 `fetch_add`/`notify` 就是 use-after-free——thread_local 只保证"活得比 op 久"、不保证"活得比 reactor 对它最后一次触碰久"，故正确挂点是进程级永生槽（注册走冷路径 CAS、无 mutex）。reactor 收尾：存 wake 指针本地 → `result.store(release)` → wake `fetch_add(release)` + `notify_one`。`notify_one` 精确唤醒（顺带消 P6 惊群）。reactor 空闲时 futex-wait 在 inbox head、pusher notify。**不做 spin-then-wait**（性能优化、首轮不做）。（本条为 P7 收敛期修 UAF 时、从最初 thread_local 唤醒字设计修订而来。） |
...
- 同步包异步唤醒（P7-D6）：进程级 WakeSlot 槽 + 结果槽分离、`notify_one`、reactor 收尾序列、
...
**范围**：
- 入站 MPSC 队列（CAS release-push / acquire-drain）自 M1 即按多生产者就绪（P7-D5，P6 `PushWriter`
  同款，无 leader 选举）；M3 不改机制、不新增原子，仅用多调用线程 + TSAN 多轮把多生产者正确性钉死
  并在 Submit/DrainAll 补依据注释（见 M3-D1「零机制改动」）。
- 并发不丢唤醒（P7-D6，`WaitResult` 四步、per-caller 进程级 WakeSlot 槽、`notify_one`）。
...
**范围**：
- **三处一致化**（路由 / BlockId device 位 / 恢复校验三者对齐，代码层共五处原子翻转见 M4 §3.1）：
  `RouteKey` 真实化 `hash%N`、`BlockId` device 位（`Init` 与 `RebuildFromActive` 均用 `device_id`）、
  `ValidateRecoveredMeta` 清"非零即拒"，外加 `Open` 放开 `size>1`（P7-D7）。
...
- by-construction 隔离论证（share-nothing、跨 device 零通信）+ **可控明显故障的轻量演示**（坏设备
  Open 被干净拒开、不污染好设备、好设备可独立打开服务），**不注入运行时 I/O 故障**（no-fault-injection）。
- P7 收敛稿（收口 P7-D1~D13 决策回链、五里程碑、退出条件、与 P11 分界）。

**退出条件**：① 明显故障（坏设备 Open）被干净拒开、不污染好设备，好设备可独立打开服务；运行时隔离由
share-nothing by-construction 保证（隔离演示）；
...
3. **唤醒挂点必须比 op 节点活得久**：reactor 在 `result.store` 之后 notify，而调用线程可能已读到结果、
   返回、栈帧（含 OpNode）析构——所以挂点放进程级长寿 WakeSlot 槽（非 thread_local——thread_local 寿命仅 = 线程，正是快路径 UAF 根因）、不放 per-op 栈节点（P7-D6，迁移 P6
   "最后触碰条款"）。
