...
- `IoBackend` 写入接口；
- `io_uring` 注册缓冲区；
- 设备、池、槽位和代次等后端中立身份传递；注册索引映射由后端维护。
...
- `ValueBuffer` 公开语义不变，P9 在 `AllocateValueBuffer(key)` 背后的池中使用 Cabe 自管 SPDK DMA 可用内存；
- 上层应用不需要知道 SPDK 分配细节；
- 应用端自备普通内存在 SPDK 后端下复制回退，不新增 DMA 内存导入接口；
- 公开 `Put` 入口不变；
...
| 旧打开周期槽位被误识别 | 错用旧池槽位或旧后端注册状态 | P8M2 采用严格 `Close()` 等待和当前池身份校验，避免旧周期资源进入新周期。 |
...
7. 为 P8M3 / P8M4 保留的设备归属和后端中立槽位身份。
