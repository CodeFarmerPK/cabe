...
| 键路由感知 | 分配 Cabe 值缓冲区时使用 key 计算目标设备，使缓冲区归属与后续 `Put` 的目标设备一致。 |
...
| 决策点 | 结论 |
| --- | --- |
| P8-D1 术语边界 | 采用“值内存 / Cabe 值缓冲区 / 值缓冲区池 / 零拷贝写入路径 / 复制回退路径 / 键路由感知”。 |
...
| P8-D6 设备归属与键路由 | `ValueBuffer` 按 key 分配并绑定目标设备；若后续 `Put` 路由到不同设备，则复制回退。 |
...
| value 来自 Cabe 值缓冲区，且目标设备匹配 | 走主零拷贝路径。 |
| value 来自 Cabe 值缓冲区，但目标设备不匹配 | 复制回退。 |
| value 来自应用自备内存，且满足当前后端零拷贝条件 | 条件式零拷贝。 |
| value 来自应用自备内存，但不满足当前后端零拷贝条件 | 复制回退。 |
...
| key 路由导致缓冲区归属设备不匹配 | 写入错误设备或错误使用注册缓冲区 | 分配接口要求传入 key；`Put` 再次校验目标设备。 |
