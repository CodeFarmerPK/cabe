...
> **状态**:**v1.0 / 已实施(P4 全部完成)** · 2026-04-28 定稿 / 2026-05-14 收尾
> **实施进度**(2026-05-14):M1 ✅ / M2 ✅ / M3 ✅ / M4 ✅ / M5 ✅ / M6 ✅ / M7 ✅ /
> **M8 ✅(评估 = 不做,Model B 在 P6 reactor 阶段被 per-thread ring 架构替代;
> 详见 §13 M8 决策结论)** / **M9 ✅(P4 收尾完成)**。
...
> M8 评估闭环 = 不做(架构定位错配 + 单设备带宽已饱和 + P6 reactor 直接绕过);
> M9 落地 `bench/baselines/p4-summary.md` + README/roadmap → ✅ + P4 实现摘要。
> 详细进度记录见 `memory/project_roadmap.md` 的 "P4 实现摘要" 章节。
...
**范围**(已落地):
- `bench/baselines/p4-summary.md` 创建,汇总 P4 各档 bench 归档:
    - `p4-pre-fixed`(M3 朴素 io_uring,无 FIXED 无 register_files)
    - `p4-post-fixed`(M4 + register_buffers + WRITE_FIXED / READ_FIXED)
    - `p4-post-fixed-files`(M5 + register_files + IOSQE_FIXED_FILE)
    - `p4-post-batch`(M7 + WriteBlocks/ReadBlocks 批量,bench 待用户 Linux 端跑)
- `README.md` Roadmap 表 P4 → ✅ 完成
- `memory/project_roadmap.md`(项目内 + Claude user memory 两端)加 "P4 实现摘要"
- `doc/p4_io_uring_design.md` 顶部状态 → v1.0 已实施;§3 决策表 / 附录 A
  D7 D14 状态符号同步;§13 M8 决策结论 + §13 M9 ✅;§18.2 修正措辞;§20 Open Questions 收口
- `io/backends/io_uring_io_backend.{h,cpp}` 顶部注释 → "P4 已完成,M8 决策记录"

**验收**(已通过):
- 19 决策 + 12 风险 + 7 衍生约束(P3 Q1-Q7)全部闭环
- M1-M7 代码 + 测试落地;M8 评估 = 不做;M9 文档/归档完成
- IoBackendTraits concept 对 sync 与 io_uring 后端 `static_assert` 通过
- 路线图状态:P4 → ✅ 完成,下一阶段 P5 WAL + 崩溃恢复
...
(落地阶段可能浮出的问题,先记下;M9 收尾时全部闭环)

- ~~`io_uring_sq_depth` 默认值是 64 还是 128?M5/M6 跑 sweep 后确定(D7)~~
  → **闭环**:M6 锁定为 64(Model A 1:1 不影响功能,M7 batch 上限 = pool_count;
  典型 pool_count=8,sq_depth=64 给充足 headroom)
- ~~M8 是否做?M7 bench 数据决定(D14、D18)~~
  → **闭环**:不做。理由见 §13 M8 决策结论。Model B 在 P6 reactor 阶段被
  per-thread ring 架构直接替代,不是 P6 的中间形态
- ~~`feature gate` 具体 try_compile 哪些宏?D10 给了方向,M1 落地时根据 liburing 2.9
  的实际 API 确定具体清单~~
  → **闭环**:M6 落地 `CABE_HAVE_IORING_SETUP_SINGLE_ISSUER` /
  `CABE_HAVE_IORING_SETUP_DEFER_TASKRUN`(Fedora 43 kernel 6.17.8 + liburing 2.9-2
  均探测到 = 1),给 P6 reactor 单生产者独占 ring 路径预留接入点
- ~~README "Production deployment notes" 章节文案细节(M6 落地)~~
  → **闭环**:M6 落地;涵盖 ulimit / systemd LimitMEMLOCK / Docker seccomp /
  Hardened kernel 不支持环境列表 / `Options.io_uring_sq_depth` 配置示例

**P4 实施阶段全部 open question 已收口,无悬空设计点**。
