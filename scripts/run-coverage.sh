...
示例:
  ...
  ./scripts/run-coverage.sh --backend=sync \
      --device=/dev/loop0 --wal-device=/dev/loop1 --snapshot-device=/dev/loop2 \
      --device2=/dev/loop3 --wal-device2=/dev/loop4 --snapshot-device2=/dev/loop5 --strict
